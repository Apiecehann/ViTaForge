...
# 2x2x50 data collection for envs/roughness_regrasp.py.
#
# Goal
# ----
...
# Output layout
# -------------
#   data_${MODALITY}/roughness_regrasp/rough_left/grasp_left/
#   data_${MODALITY}/roughness_regrasp/rough_left/grasp_right/
#   data_${MODALITY}/roughness_regrasp/rough_right/grasp_left/
#   data_${MODALITY}/roughness_regrasp/rough_right/grasp_right/
#
# Examples
# --------
#   MODALITY=gelsight CONFIG=task_config/gelsight.yml GPU=0 bash bash_scripts/collect_roughness_regrasp_2x2x50.sh
#   MODALITY=xense CONFIG=task_config/xense.yml GPU=1 bash bash_scripts/collect_roughness_regrasp_2x2x50.sh
#
#   bash bash_scripts/collect_roughness_regrasp_2x2x50.sh
#   DRY_RUN=1 bash bash_scripts/collect_roughness_regrasp_2x2x50.sh
#   EPISODES_PER_CASE=20 MAX_SEED=199 bash bash_scripts/collect_roughness_regrasp_2x2x50.sh
#   SKIP_CONDA=1 bash bash_scripts/collect_roughness_regrasp_2x2x50.sh
...
RUN_NAME="${RUN_NAME:-roughness_regrasp_2x2x50_$(date +%Y%m%d_%H%M%S)}"
OUTPUT_ROOT="${OUTPUT_ROOT:-${PROJECT_ROOT}/data_${MODALITY}}"
LOG_ROOT="${LOG_ROOT:-${PROJECT_ROOT}/logs/collect_roughness_regrasp_2x2x50}"
...
TEMP_CONFIG_DIR="$(mktemp -d /tmp/univtac-roughness-regrasp-2x2x50.XXXXXX)"
cleanup() {
    if [[ "${TEMP_CONFIG_DIR}" == /tmp/univtac-roughness-regrasp-2x2x50.* ]]; then
...
run_one() {
    ...
    {
        printf '\n'
        printf '# Added by bash_scripts/collect_roughness_regrasp_2x2x50.sh\n'
