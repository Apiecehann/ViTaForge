...
# Balanced data collection for envs/insert_block.py.
#
# Goal
# ----
...
# -----------------------
# The default is:
#   3 targets x 4 layouts x 20 successful episodes = 240 successful episodes
# which gives 80 successful episodes per target.
#
# Examples
# --------
#   MODALITY=gelsight CONFIG=task_config/gelsight.yml GPU=0 bash bash_scripts/collect_insert_block_balanced.sh
#   MODALITY=xense CONFIG=task_config/xense.yml GPU=1 bash bash_scripts/collect_insert_block_balanced.sh
#   MODALITY=neote CONFIG=task_config/neote.yml GPU=2 bash bash_scripts/collect_insert_block_balanced.sh
#
#   bash bash_scripts/collect_insert_block_balanced.sh
#   DRY_RUN=1 bash bash_scripts/collect_insert_block_balanced.sh
#   EPISODES_PER_LAYOUT=20 MAX_SEED_OFFSET=399 \
#     bash bash_scripts/collect_insert_block_balanced.sh
#   SKIP_CONDA=1 bash bash_scripts/collect_insert_block_balanced.sh
...
#   data/insert_block/${MODALITY}/{cube,half_cylinder,hexagon}/
...
#   TARGETS="cube hexagon" bash bash_scripts/collect_insert_block_balanced.sh
TARGETS="${TARGETS:-cube half_cylinder hexagon}"

# Each target is collected under each layout. With the default 4 layouts,
# EPISODES_PER_LAYOUT=20 means each target receives 80 successful episodes.
...
RUN_NAME="${RUN_NAME:-insert_block_balanced_$(date +%Y%m%d_%H%M%S)}"
OUTPUT_ROOT="${OUTPUT_ROOT:-${PROJECT_ROOT}/data}"
LOG_ROOT="${LOG_ROOT:-${PROJECT_ROOT}/logs/collect_insert_block_balanced}"
...
# Four compact benchmark layouts. Layout entries use TARGET_BLOCKS order:
#   (cube, half_cylinder, hexagon)
LAYOUTS=(
    "layout0:0,1,4"
    "layout1:1,3,2"
...
TEMP_CONFIG_DIR="$(mktemp -d /tmp/univtac-insert-block-balanced.XXXXXX)"
cleanup() {
    if [[ "${TEMP_CONFIG_DIR}" == /tmp/univtac-insert-block-balanced.* ]]; then
...
run_one() {
    ...
    local output_dir="${OUTPUT_ROOT}/insert_block/${MODALITY}/${target}"
    ...
    {
        printf '\n'
        printf '# Added by bash_scripts/collect_insert_block_balanced.sh\n'
...
echo "Output directories:   ${OUTPUT_ROOT}/insert_block/${MODALITY}/{cube,half_cylinder,hexagon}"
