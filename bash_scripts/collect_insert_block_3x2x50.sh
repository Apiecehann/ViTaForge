...
# 3x2x50 data collection for envs/insert_block.py.
#
# Goal
# ----
# Collect trajectories for all three insertable objects:
#   cube, half_cylinder, hexagon
#
# This script treats two factors as explicit data-collection variables:
#   1. target_block: which object the scripted policy should grasp and insert.
#   2. layout: which fixed anchor each object starts from before per-episode
#      small xy jitter is applied by envs/insert_block.py.
#
# Why layout is enumerated outside the environment
# -----------------------------------------------
# insert_block reset currently moves each actor back to its layout anchor plus
# small xy noise. We intentionally do not reshuffle large object positions inside
# a single Python/IsaacSim process, because doing that during reset can create
# large instantaneous moves and object collisions. Instead, every target/layout
# pair is collected in a separate collect_data.py run, so create_actors() builds
# the scene with the intended layout from the start.
#
# Anchor indices in envs/insert_block.py
# -------------------------------------
#   0: (0.45, 0.15, 0.002)
#   1: (0.37, 0.08, 0.002)
#   2: (0.36, 0.24, 0.002)
...
#   4: (0.54, 0.24, 0.002)
#
# Layout entries use TARGET_BLOCKS order from envs/insert_block.py:
#   (cube, half_cylinder, hexagon)
#
# Default collection size
# -----------------------
# The default is:
#   3 targets x 2 layouts x 50 successful episodes = 300 successful episodes
# which gives 100 successful episodes per target.
#
# Examples
# --------
#   MODALITY=gelsight CONFIG=task_config/gelsight.yml GPU=0 bash bash_scripts/collect_insert_block_3x2x50.sh
#   MODALITY=xense CONFIG=task_config/xense.yml GPU=1 bash bash_scripts/collect_insert_block_3x2x50.sh
#   MODALITY=neote CONFIG=task_config/neote.yml GPU=2 bash bash_scripts/collect_insert_block_3x2x50.sh
#
#   bash bash_scripts/collect_insert_block_3x2x50.sh
#   DRY_RUN=1 bash bash_scripts/collect_insert_block_3x2x50.sh
#   EPISODES_PER_LAYOUT=20 MAX_SEED_OFFSET=399 \
#     bash bash_scripts/collect_insert_block_3x2x50.sh
#   SKIP_CONDA=1 bash bash_scripts/collect_insert_block_3x2x50.sh
...
# Run one tactile modality at a time. Data is grouped by target object under:
#   data_${MODALITY}/insert_block/{cube,half_cylinder,hexagon}/
...
# Space-separated subset is supported, for example:
#   TARGETS="cube hexagon" bash bash_scripts/collect_insert_block_3x2x50.sh
TARGETS="${TARGETS:-cube half_cylinder hexagon}"

# Each target is collected under each layout. With the default 2 layouts,
# EPISODES_PER_LAYOUT=50 means each target receives 100 successful episodes.
EPISODES_PER_LAYOUT="${EPISODES_PER_LAYOUT:-50}"
START_SEED="${START_SEED:-0}"

# Every layout for the same target writes into the same target directory, and
# hdf5 files are named by seed. To avoid overwriting hdf5/<seed>.hdf5 between
# layouts, this script gives each layout a non-overlapping seed range:
...
MAX_SEED_OFFSET="${MAX_SEED_OFFSET:-${MAX_SEED:-999}}"
SEED_STRIDE="${SEED_STRIDE:-1000}"

RUN_NAME="${RUN_NAME:-insert_block_3x2x50_$(date +%Y%m%d_%H%M%S)}"
OUTPUT_ROOT="${OUTPUT_ROOT:-${PROJECT_ROOT}/data_${MODALITY}}"
LOG_ROOT="${LOG_ROOT:-${PROJECT_ROOT}/logs/collect_insert_block_3x2x50}"
RUN_LOG_DIR="${LOG_ROOT}/${RUN_NAME}"
SUMMARY_FILE="${RUN_LOG_DIR}/summary.tsv"

# Layout format: "layout_name:cube_anchor,half_cylinder_anchor,hexagon_anchor".
# Two compact benchmark layouts. Layout entries use TARGET_BLOCKS order:
#   (cube, half_cylinder, hexagon)
LAYOUTS=(
    "layout0:0,1,4"
    "layout1:1,3,2"
...
validate_target() {
    case "$1" in
        cube|half_cylinder|hexagon)
            ...
            ;;
        *)
            echo "Unknown target_block: $1"
            echo "Valid targets: cube half_cylinder hexagon"
            return 1
            ;;
    esac
}
...
TEMP_CONFIG_DIR="$(mktemp -d /tmp/univtac-insert-block-3x2x50.XXXXXX)"
cleanup() {
    if [[ "${TEMP_CONFIG_DIR}" == /tmp/univtac-insert-block-3x2x50.* ]]; then
        rm -rf -- "${TEMP_CONFIG_DIR}"
    fi
}
trap cleanup EXIT

printf 'modality\ttarget_block\tlayout\tpose_indices\tresult\texit_code\tsuccesses\tlog\n' > "${SUMMARY_FILE}"
...
run_one() {
    local target="$1"
    local layout_name="$2"
    local pose_indices="$3"
    local layout_index="$4"
    local config_stem="${MODALITY}"
    local run_stem="${MODALITY}_${target}_${layout_name}"
    local generated_config="${TEMP_CONFIG_DIR}/${config_stem}.yml"
    local output_dir="${OUTPUT_ROOT}/insert_block/${target}"
    ...
    # the explicit insert_block controls consumed by scripts/collect_data.py.
    sed "s|^save_dir:.*|save_dir: ${OUTPUT_ROOT}|" "${SOURCE_CONFIG}" > "${generated_config}"
    {
        printf '\n'
        printf '# Added by bash_scripts/collect_insert_block_3x2x50.sh\n'
        printf 'save_dir_exact: %s\n' "${output_dir}"
        printf 'target_block: %s\n' "${target}"
        printf 'block_base_pose_indices: [%s]\n' "${pose_indices}"
    } >> "${generated_config}"
    ...
    echo "Pose indices:  ${pose_indices}  (cube, half_cylinder, hexagon)"
    ...
    if [[ "${DRY_RUN}" == "1" ]]; then
        ...
        printf '%s\t%s\t%s\t%s\tDRY_RUN\t0\t0\t%s\n' \
            "${MODALITY}" "${target}" "${layout_name}" "${pose_indices}" "${log_file}" \
            >> "${SUMMARY_FILE}"
        ((dry_run_count += 1))
        return 0
    fi
    ...
    printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
        "${MODALITY}" "${target}" "${layout_name}" "${pose_indices}" \
...
echo "Output directories:   ${OUTPUT_ROOT}/insert_block/{cube,half_cylinder,hexagon}"
...
for target in "${target_list[@]}"; do
    ...
    for layout_entry in "${LAYOUTS[@]}"; do
        ...
        run_one "${target}" "${layout_name}" "${pose_indices}" "${layout_index}"
        ((layout_index += 1))
