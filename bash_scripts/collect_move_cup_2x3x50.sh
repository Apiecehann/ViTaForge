...
# 2x3x50 data collection for envs/move_cup.py.
#
# Collection scope
# ----------------
# This script collects two requested cup relation variants:
#   blue  left_of_yellow
#   green right_of_blue
#
# The remaining cup stays in the scene as a distractor.
#
# Layouts
# -------
...
# -----------------------
#   2 semantic variants x 3 layouts x 50 successful episodes
#   = 300 successful episodes
#
# Output layout
# -------------
#   data_${MODALITY}/move_cup/blue/left_of_yellow/
#   data_${MODALITY}/move_cup/green/right_of_blue/
#
# Examples
# --------
#   MODALITY=gelsight CONFIG=task_config/gelsight.yml GPU=0 bash bash_scripts/collect_move_cup_2x3x50.sh
#   MODALITY=xense CONFIG=task_config/xense.yml GPU=1 bash bash_scripts/collect_move_cup_2x3x50.sh
#   MODALITY=neote CONFIG=task_config/neote.yml GPU=2 bash bash_scripts/collect_move_cup_2x3x50.sh
#
#   bash bash_scripts/collect_move_cup_2x3x50.sh
#   DRY_RUN=1 bash bash_scripts/collect_move_cup_2x3x50.sh
#   EPISODES_PER_LAYOUT=10 MAX_SEED_OFFSET=99 \
#     bash bash_scripts/collect_move_cup_2x3x50.sh
#   SKIP_CONDA=1 bash bash_scripts/collect_move_cup_2x3x50.sh
...
#   data_${MODALITY}/move_cup/${target}/${side}_of_${reference}/
...
RUN_NAME="${RUN_NAME:-move_cup_2x3x50_$(date +%Y%m%d_%H%M%S)}"
OUTPUT_ROOT="${OUTPUT_ROOT:-${PROJECT_ROOT}/data_${MODALITY}}"
LOG_ROOT="${LOG_ROOT:-${PROJECT_ROOT}/logs/collect_move_cup_2x3x50}"
...
TEMP_CONFIG_DIR="$(mktemp -d /tmp/univtac-move-cup-2x3x50.XXXXXX)"
cleanup() {
    if [[ "${TEMP_CONFIG_DIR}" == /tmp/univtac-move-cup-2x3x50.* ]]; then
...
run_one() {
    ...
    local output_dir="${OUTPUT_ROOT}/move_cup/${target}/${semantic_dir}"
    ...
    {
        printf '\n'
        printf '# Added by bash_scripts/collect_move_cup_2x3x50.sh\n'
...
echo "Output directories:   ${OUTPUT_ROOT}/move_cup/{blue/left_of_yellow,green/right_of_blue}"
