...
# 4x25 gelsight data collection for envs/can_empty_select.py.
#
# Goal
# ----
# Collect four explicit empty-can cases in this order:
#   1. coke, 25 successful episodes
#   2. fanta, 25 successful episodes
#   3. 7up, 25 successful episodes
#   4. pepsi, 25 successful episodes
#
# Output layout
# -------------
#   data_gelsight/can/can_empty_select/coke/
#   data_gelsight/can/can_empty_select/fanta/
#   data_gelsight/can/can_empty_select/7up/
#   data_gelsight/can/can_empty_select/pepsi/
#
# Examples
# --------
#   GPU=0 bash bash_scripts/collect_can_empty_select_4x25.sh
#   DRY_RUN=1 bash bash_scripts/collect_can_empty_select_4x25.sh
#   EPISODES_PER_CASE=10 MAX_SEED=199 bash bash_scripts/collect_can_empty_select_4x25.sh
#   SKIP_CONDA=1 bash bash_scripts/collect_can_empty_select_4x25.sh
...
RUN_NAME="${RUN_NAME:-can_empty_select_4x25_$(date +%Y%m%d_%H%M%S)}"
OUTPUT_ROOT="${OUTPUT_ROOT:-${PROJECT_ROOT}/data_gelsight/can}"
LOG_ROOT="${LOG_ROOT:-${PROJECT_ROOT}/logs/collect_can_empty_select_4x25}"
...
validate_empty_can() {
    case "$1" in
        coke|fanta|7up|pepsi|random)
            return 0
            ;;
        *)
            echo "Unknown empty_can: $1"
            echo "Valid labels: coke fanta 7up pepsi random"
            return 1
            ;;
    esac
}
...
TEMP_CONFIG_DIR="$(mktemp -d /tmp/univtac-can-empty-select-4x25.XXXXXX)"
cleanup() {
    if [[ "${TEMP_CONFIG_DIR}" == /tmp/univtac-can-empty-select-4x25.* ]]; then
        rm -rf -- "${TEMP_CONFIG_DIR}"
    fi
}
trap cleanup EXIT

printf 'modality\tempty_can\tresult\texit_code\tsuccesses\tlog\n' > "${SUMMARY_FILE}"
...
run_one() {
    ...
    validate_empty_can "${empty_can}" || return 2

    local run_stem="${MODALITY}_${empty_can}"
    local generated_config="${TEMP_CONFIG_DIR}/${run_stem}.yml"
    local output_dir="${OUTPUT_ROOT}/can_empty_select/${empty_can}"
    ...
    {
        printf '\n'
        printf '# Added by bash_scripts/collect_can_empty_select_4x25.sh\n'
...
echo "Output dirs:        ${OUTPUT_ROOT}/can_empty_select/{coke,fanta,7up,pepsi}"
echo "Log root:           ${RUN_LOG_DIR}"
echo "Dry run:            ${DRY_RUN}"

for empty_can in "${CASES[@]}"; do
    run_one "${empty_can}"
