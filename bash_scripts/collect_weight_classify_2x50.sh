...
# 2x50 data collection for envs/weight_classify.py.
#
# Goal
# ----
# Collect two explicit cases in this order:
#   1. fixed heavy block, 50 successful episodes
#   2. fixed light block, 50 successful episodes
#
# Default collection size
# -----------------------
#   2 cases x 50 successful episodes = 100 successful episodes
#
# Output layout
# -------------
#   data_${MODALITY}/weight_classify/heavy/
#   data_${MODALITY}/weight_classify/light/
#
# Examples
# --------
#   MODALITY=gelsight CONFIG=task_config/gelsight.yml GPU=0 bash bash_scripts/collect_weight_classify_2x50.sh
#   MODALITY=xense CONFIG=task_config/xense.yml GPU=1 bash bash_scripts/collect_weight_classify_2x50.sh
#
#   bash bash_scripts/collect_weight_classify_2x50.sh
#   DRY_RUN=1 bash bash_scripts/collect_weight_classify_2x50.sh
#   EPISODES_PER_CASE=20 MAX_SEED=199 bash bash_scripts/collect_weight_classify_2x50.sh
#   SKIP_CONDA=1 bash bash_scripts/collect_weight_classify_2x50.sh
...
RUN_NAME="${RUN_NAME:-weight_classify_2x50_$(date +%Y%m%d_%H%M%S)}"
OUTPUT_ROOT="${OUTPUT_ROOT:-${PROJECT_ROOT}/data_${MODALITY}}"
LOG_ROOT="${LOG_ROOT:-${PROJECT_ROOT}/logs/collect_weight_classify_2x50}"
RUN_LOG_DIR="${LOG_ROOT}/${RUN_NAME}"
SUMMARY_FILE="${RUN_LOG_DIR}/summary.tsv"

# Case order is intentional: collect heavy first, then light.
CASES=(
    "heavy"
    "light"
)
...
TEMP_CONFIG_DIR="$(mktemp -d /tmp/univtac-weight-classify-2x50.XXXXXX)"
cleanup() {
    if [[ "${TEMP_CONFIG_DIR}" == /tmp/univtac-weight-classify-2x50.* ]]; then
...
run_one() {
    ...
    local output_dir="${OUTPUT_ROOT}/weight_classify/${weight_label}"
    ...
    {
        printf '\n'
        printf '# Added by bash_scripts/collect_weight_classify_2x50.sh\n'
...
echo "Output dirs:        ${OUTPUT_ROOT}/weight_classify/{heavy,light}"
