...
class ManiSkillSimulator(GelSightSimulator):
    ...
    def marker_motion_simulation(self):
        marker_flow = torch.as_tensor(
            self.marker_motion_sim.gen_marker_flow(),
            dtype=self.marker_data.dtype,
            device=self.marker_data.device,
        )
        gain = float(self.cfg.marker_motion_gain)
        if gain != 1.0:
            marker_flow = torch.stack(
                (marker_flow[0], marker_flow[0] + gain * (marker_flow[1] - marker_flow[0])),
                dim=0,
            )
