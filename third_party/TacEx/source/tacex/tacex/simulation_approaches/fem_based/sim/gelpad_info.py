...
def get_gelpad_info(mesh:SimplicialComplex):
    ...
    def triangle_area_xy(face_pts):
        edge_1 = face_pts[1, :2] - face_pts[0, :2]
        edge_2 = face_pts[2, :2] - face_pts[0, :2]
        return abs(edge_1[0] * edge_2[1] - edge_1[1] * edge_2[0]) * 0.5

    # Marker binding should use the actual gelpad top skin. The Xense tensor pad
    # mesh has bevel/edge triangles close to max-z; including them makes some
    # marker triangles degenerate after camera projection.
    surface_candidates = []
    fallback_surface_candidates = []
    for idx, norm in enumerate(faces_norm):
        face = faces[idx]
        face_pts = points[face]
        on_top_band = np.all(face_pts[:, 2] >= mxz - 2e-3)
        has_projected_area = triangle_area_xy(face_pts) > 1e-12
        if on_top_band and has_projected_area:
            fallback_surface_candidates.append(idx)
            if np.dot(norm, [0, 0, 1]) > 0.8:
                surface_candidates.append(idx)
        if np.dot(norm, [0, 0, -1]) > 0.99 and np.all(face_pts[:, 2] <= miz+1e-4):
            faces_on_bottom.append(face)
            vertices_on_bottom.extend(face)

    if not surface_candidates:
        surface_candidates = fallback_surface_candidates

    for idx in surface_candidates:
        face = faces[idx]
        faces_on_surface.append(face)
        vertices_on_surface.extend(face)
