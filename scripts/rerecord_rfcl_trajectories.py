...
def load_trajectory(path: Path) -> dict[str, Any]:
    ...
    result["episode"] = int(np.asarray(result["episode"]).item())
...
def write_replay_metadata(
    ...
    *,
    ...
    with h5py.File(path, "a") as handle:
        handle.attrs["rfcl_rerecord_schema"] = "rfcl_rgb_tactile_rerecord_v1"
        handle.attrs["rfcl_replay_success"] = True
        handle.attrs["rfcl_source_trajectory"] = str(source.resolve())
        handle.attrs["rfcl_source_episode"] = int(trajectory["episode"])
...
def record_attempt(
    ...
    *,
    ...
    task.metadata.update(
        {
            ...
            "rfcl_source_episode": int(trajectory["episode"]),
    ...
    validation = validate_hdf5(partial_path, source=source)
    ...
    try:
        ...
        for selection_index, source in enumerate(trajectories):
            ...
            if args.resume and final_path.is_file():
                try:
                    validation = validate_hdf5(final_path, source=source)
