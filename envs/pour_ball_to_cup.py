...
# 场景初始状态。
BLUE_CUP_BASE_POSE = Pose([0.50, 0.10, 0.002], (1.0, 0.0, 0.0, 0.0))
YELLOW_CUP_BASE_POSE = Pose([0.50, -0.10, 0.002], (1.0, 0.0, 0.0, 0.0))
RED_BALL_BASE_POSE = Pose([0.50, 0.10, 0.02], (1.0, 0.0, 0.0, 0.0))
BALL_BOTTOM_Z = 0.005
...
# 动作参数。
INITIAL_SETTLING_STEPS = 10
DEFAULT_JOINT7_ROTATION_STEPS = 160
MOTION_TIME_DILATION_FACTOR = 0.5
ZERO_TRANSLATION_TOLERANCE = 1e-6
BLUE_CUP_GRASP_X_OFFSET = 0.010
CUP_HEIGHT = 0.0925
CUP_RIM_HEIGHT = 0.011
PRE_GRASP_HEIGHT = 0.050
CARRY_Y_OFFSET = 0.030
CARRY_HEIGHT = 0.120
POUR_JOINT7_DELTA = np.deg2rad(120.0)
POUR_EE_TRANSLATION = (0.0, 0.0, 0.0)
BALL_STILL_MAX_STEPS = 600
BALL_STILL_POSITION_TOLERANCE = 1e-4
BALL_STILL_QUATERNION_TOLERANCE = 1e-3

# 成功判定参数。
SUCCESS_CUP_RADIUS = 0.068 * 0.5
SUCCESS_CUP_HEIGHT = 0.096
...
class Task(BaseTask):
    ...
    def load_robot_and_sensors(self, cfg: BaseTaskCfg):
        ...
        self._use_tilted_home_joint_pos = bool(TASK_INITIAL_ARM_JOINT_POS)
    ...
    def create_actors(self):
        ...
        self.yellow_cup = self._actor_manager.add_from_usd_file(
            ...
            pose=YELLOW_CUP_BASE_POSE,
            density=1e5,
        )
        self.red_ball = self._actor_manager.add_from_usd_file(
            name="red_ball",
            asset_path="ball_red.usd",
            # 红球跟蓝杯同 x/y; 球心高度 0.02m = 杯底 z 0.002m + 杯底厚约 0.002m + 预留 0.001m + 半径 0.015m。
            pose=RED_BALL_BASE_POSE,
            density=200,
        )

    def _reset_actors(self):
        # 蓝杯和红球共用一组 xy 扰动，保证红球仍在蓝杯内；黄杯使用独立扰动。
        blue_cup_offset = self.create_noise(list(CUP_RESET_XY_NOISE))
        yellow_cup_offset = self.create_noise(list(CUP_RESET_XY_NOISE))
        blue_cup_pose = BLUE_CUP_BASE_POSE.add_offset(blue_cup_offset)
        yellow_cup_pose = YELLOW_CUP_BASE_POSE.add_offset(yellow_cup_offset)
        red_ball_pose = RED_BALL_BASE_POSE.add_offset(blue_cup_offset)
        self.blue_cup.set_pose(blue_cup_pose)
        self.yellow_cup.set_pose(yellow_cup_pose)
        self.red_ball.set_pose(red_ball_pose)
        self.metadata["blue_cup_and_red_ball_xy_noise"] = blue_cup_offset.p.tolist()
        self.metadata["yellow_cup_xy_noise"] = yellow_cup_offset.p.tolist()
        ...
        self.metadata["ball_bottom_z"] = BALL_BOTTOM_Z

    def pre_move(self):
        # 先等待 10 step 看初始 settling。夹爪开口已经在 init_state 里设成最大值, 不再额外规划 open_gripper。
        self.delay(INITIAL_SETTLING_STEPS)
    ...
    def _rotate_last_arm_joint(
        self,
        delta: float,
        steps: int = DEFAULT_JOINT7_ROTATION_STEPS,
        tag: str = "rotate_last_joint",
        is_save: bool = True,
    ):
    ...
    def _rotate_last_arm_joint_with_ee_translation(
        ...
        is_save: bool = True,
        time_dilation_factor: float = MOTION_TIME_DILATION_FACTOR,
    ):
        # 同一个动作里做两件事:
        ...
        if np.linalg.norm(translation) < ZERO_TRANSLATION_TOLERANCE:
    ...
    def _wait_ball_until_still(
        self,
        max_steps: int = BALL_STILL_MAX_STEPS,
        pos_tol: float = BALL_STILL_POSITION_TOLERANCE,
        quat_tol: float = BALL_STILL_QUATERNION_TOLERANCE,
    ...
    def _is_ball_in_yellow_cup(self):
        ...
        cup_radius = SUCCESS_CUP_RADIUS
        cup_height = SUCCESS_CUP_HEIGHT
    ...
    def _play_once(self):
        ...
        grasp_pos = cup_pose.p + np.array([
            BLUE_CUP_GRASP_X_OFFSET,
            0.0,
            CUP_HEIGHT - 0.5 * CUP_RIM_HEIGHT,
        ])
        ...
        pre_grasp_gripper_center_pose = Pose(
            grasp_pos + np.array([0.0, 0.0, PRE_GRASP_HEIGHT]),
            tilted_eef_q,
        )
        ...
        self.move(
            ...
            time_dilation_factor=MOTION_TIME_DILATION_FACTOR,
        )
        ...
        target_blue_cup_pos = yellow_cup_pose.p + np.array([0.0, CARRY_Y_OFFSET, CARRY_HEIGHT])
        ...
        self._rotate_last_arm_joint_with_ee_translation(
            delta=POUR_JOINT7_DELTA,
            translation=np.array(POUR_EE_TRANSLATION),
            steps=DEFAULT_JOINT7_ROTATION_STEPS,
